...
class biRNN(object):
    ...
    def calculate_cost(self):
        ...
        words_indexes = tf.argmax(words_logits, axis=1)
        labels_indexes = tf.argmax(labels_logits, axis=1)
        words_indexes = tf.reshape(words_indexes, [batch_size, -1])
        labels_indexes = tf.reshape(labels_indexes, [batch_size, -1])
        self.best_indexes = tf.concat([words_indexes, labels_indexes], axis=1)
